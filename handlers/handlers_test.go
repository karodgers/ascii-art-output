...
func TestExtractTextAndFilename(t *testing.T) {
	tests := []struct {
		name         string
		args         []string
		wantText     string
	...
	}{
		{
			...
			args:         []string{"--output=banner.txt", "thinkertoy"},
			wantText:     "",
			wantFilename: "banner.txt",
			wantError:    false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			text, filename := ExtractTextAndFilename(tt.args)
			if text != tt.wantText {
